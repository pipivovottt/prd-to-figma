...
### 6. 数字文本 Number 字体强制替换（不可省略）

> 此规则来自 `text-style-ref.md` 全局强制规则，适用于移动端**所有页面、所有场景**中显示数字的文字节点，不限于特定组件或页面类型。

**时机**：向任意数字类文字节点写入内容（`text.set_content`）后，必须立即按 `text-style-ref.md` 流程 1b 执行 `text.update`。样式 Key 见 `text-style-ref.md` 移动端组件库表格。

**移动端中常见的数字节点场景（示例，非穷举）**：

| 页面 / 区域 | 数字内容示例 | 典型节点位置 |
|------------|------------|------------|
| 列表页 - 基金列表 | `+3.25%`、`1.5823` | 涨跌幅、净值数字节点 |
| 详情页 - 资产卡片 | `¥12,000.00`、`+¥580.00` | 收益金额节点 |
| 详情页 - 数据行 | `2024/03/15`、`88%` | 日期、百分比节点 |
| 首页 - 资产概览 | `¥1,234,567.89` | 总资产大数字节点 |
| 任意页面 - 编号 / 计数 | `NO.001`、`1/6` | 编号、计数文字节点 |
...
2. **非组件元素样式规范**：
   - **文本样式**：所有文本节点（含组件内部）必须使用组件库中预设的文本样式（Key 及操作流程参见 `text-style-ref.md`）；其中显示数字内容的节点，无论是否在组件内，都必须使用 Number 系列样式（详见步骤 5 及 `text-style-ref.md` 流程 1b）
