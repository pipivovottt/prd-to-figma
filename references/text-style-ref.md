...
- 若返回错误 `No channel joined` → **停止操作**，提示用户：
  > 请在 Figma 中打开 Vibma 插件，频道名填写 `agent`，点击 Connect 后告知我，再继续操作。
...
**核心原则**：页面中所有包含数字的**非组件独立文本节点**，必须使用组件库预设的 **Number** 系列文本样式（MiNum 等宽字形），不能使用普通中文（CN）或英文（EN）样式。
...
**为什么使用 Number 样式**：Number 样式使用等宽数字字形（Tabular Figures），保证表格/列表中数字列对齐，提升金融数据可读性。
...
> ⚠️ **组件内部的数字文本**（如表格单元格、列表项内的数字）已由组件库预设，**不需要另行修改**，通过 `text.set_content` 更新内容即可，样式随组件保留。
